...
var joinParameters = function(url, baseParam, targetParam){
	var baseParamVal = getParameterByName(baseParam);
	var targetParamVal = getParameterByName(targetParam);
	var result = baseParam + baseParamVal + '-' + targetParam + '-' + targetParamVal;
	var joinedParams =  updateParam(url, baseParam, result);
	return result;
}
    ...
    for(var i = 0; links.length > i; i++){
        ...
        if (linkURL.indexOf(str) != -1){
        	...
        	if(joinParams != undefined){
        		joinParameters(joinParams[0], joinParams[1]);
        	}
		    console.log(linkURL);    
        }
	}
