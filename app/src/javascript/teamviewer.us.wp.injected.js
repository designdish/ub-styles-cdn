...
var injectSitePhoneNumber = function() {
    if (getHeroContainer(heroText)) {
        ...
        if (heroInnerColumn != undefined) {
            heroInnerColumn.insertAdjacentHTML('beforeend', featureContactNumber.innerHTML);

        }

        if (currentPage.indexOf('enterprise') !== -1) {
            var entModalTrigger = 'Interested in Identity Provider Connection (IdP) early access? <span class="idp-modal-trigger">Let us know</span>';
            heroInnerColumn.insertAdjacentHTML('beforeend', entModalTrigger);
        }
    }
};
